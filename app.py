...
import os
import logging
from chalicelib import aws_functions, bitwarden_functions
...
def handler(event, context):
    ...
    try:
        ...
        logging.info(f"/tmp/bw exists and is writable: {os.access('/tmp/bw', os.W_OK)}")
    except Exception as e:
        logging.error(f"Failed to create /tmp/bw: {e}")

    logging.info("About to call get_session()")
    try:
        session = bitwarden_functions.get_session(env)
        logging.info(f"get_session() returned successfully, session length: {len(session) if session else 'None'}")
    except Exception as e:
        logging.error(f"get_session() raised an exception: {e}")
        raise
    ...
    try:
        ...
        member = bitwarden_functions.invite_org_member(
            env, ORGANIZATION_ID, user_email, BW_ORG_CLIENT_ID, BW_ORG_CLIENT_SECRET,
        )
        ...
        collection = bitwarden_functions.grant_user_collection_access(
            env, ORGANIZATION_ID, collection, user_email, member=member,
        )
...
# Test command
# handler({"data": {"email": "jilliam.sagun11@safe.com"}}, "tasa")